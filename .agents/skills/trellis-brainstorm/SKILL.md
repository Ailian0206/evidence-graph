...
A request to build, implement, fix, refactor, or "go ahead" authorizes routine planning and implementation after the artifacts pass Agent self-review. Create Trellis tasks without a separate process confirmation.
...
Use this skill after a complex task or milestone has been identified.
...
8. Self-review the final planning summary. If no explicit gate in `AGENT.md` is pending, run `task.py start` and continue implementation in the same turn.
...
Do not manufacture clarification questions when the request and repository evidence already resolve every decision. In that case, proceed directly through self-review and implementation.
...
Lightweight tasks may omit `design.md` and `implement.md`; they may not skip evidence inspection, requirement convergence, or final self-review.
