...
`get_context.py` shows the active task's `status` field. Route by `status` + artifact presence. Routine phase transitions proceed after Agent self-review; ask only for the explicit confirmation gates in `AGENT.md`.
...
- `status=planning` + required artifacts complete + required jsonl curated or inline mode → **1.4** (self-review, then run `task.py start`)
