...
| Main Codex process | Continue non-overlapping local module work, then review and integrate completed Autofix output. | Must not edit code owned by an active Autofix run |
...
- Start one read-only monitor subagent for that PR. It records the PR number, head SHA, CI state, latest Bugbot review SHA, Autofix run or branch, and Cursor-authored commits.
...
- Autofix ownership ends when Cursor publishes a commit or patch, reports a terminal failure, or no longer has an active run for the latest finding.
...
- Merge the module PR with a merge commit only after CI is green, Autofix output has been reviewed, and no valid finding remains unresolved.
- A Bugbot usage-limit skip is missing review coverage, not a code finding. Record it in `PROJECT_STATUS.md`, do not repeatedly retrigger paid reviews, and keep local work moving. The module PR remains open unless the user explicitly accepts a CI-only merge.
...
Use one monitor subagent per active module PR and close it after it reports a terminal or actionable state. Start another monitor only after a meaningful PR head or check-state change.
