...
- 无效 finding 由 Codex 记录技术依据并拒绝修改；Claude 标签只能由 reviewer 更新，Codex 不手工操作。
...
每条 summary 评论以当前 PR head SHA 结尾：
...
标签缺失、marker 缺失或 SHA 过期都表示审核未完成。Claude 非零退出时检查认证和输出并自动重试一次；第二次仍失败则保持 PR 未审核，主进程继续其它不冲突任务，但不得合并。
