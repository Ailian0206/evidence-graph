...
所有 Claude 命令都必须带 `--setting-sources project,local`，确保加载当前仓库的 reviewer，而不是 `~/.claude/skills/` 中其它项目的同名 skill。
...
```bash
claude --setting-sources project,local --permission-mode auto --model sonnet -p "/pr-review --recheck <PR编号>"
```
