...
- DeepSeek `deepseek-chat`：规划搜索、抽取主张、关联证据、识别冲突和生成报告。
...
6. 独立付费冒烟命令、精确确认令牌和 `0.10 USD` 首次冒烟上限。
7. Vercel Production 环境变量配置和一次线上真实研究验收。

### 不包含

- 不新增数据库迁移、队列系统或 Provider SDK。
...
- 使用 `deepseek-chat` 的 Chat Completions JSON Object 输出。
...
7. 合并后配置 Production，创建一个低范围中文研究并检查 ready、来源、主张、证据、报告和费用记录。
