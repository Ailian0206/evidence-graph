...
| 9 | `feat/report-publishing` | 报告发布、撤销、公开 slug、版本切换和分享页 | 最新 `main` 基线门禁通过 89 个数据库测试、170 个单元测试和 45 个 E2E；准备创建唯一 Draft PR |
...
`feat/managed-deployment` 与 `feat/durable-research-results` 已依次通过 PR #13、#14 合并，Vercel 生产验证保留为后置发布门禁。`feat/report-publishing` 已在最新 `main` 基线上通过完整门禁，准备创建唯一 Draft PR；闭环后再开始 `feat/ui-experience-refresh`。
