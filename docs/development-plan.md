...
`feat/managed-deployment`、`feat/durable-research-results` 与 `feat/report-publishing` 已依次通过 PR #13、#14、#15 合并，Vercel 生产验证保留为后置发布门禁。当前从最新 `main` 开始 `feat/ui-experience-refresh`，先完成全站界面审计和统一设计规范，再集中实现并创建唯一 Draft PR。
