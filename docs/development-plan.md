...
| 11 | `feat/real-provider-integration` | Production 直连 Tavily、DeepSeek 与百炼，开发和 CI 保持 fixtures | 本地完成；TDD、低成本真实冒烟和完整模块门禁已通过，等待 Draft PR、独立审核与 CI |
...
`feat/managed-deployment`、`feat/durable-research-results`、`feat/report-publishing` 与 `feat/ui-experience-refresh` 已依次通过 PR #13、#14、#15、#16 合并。`feat/real-provider-integration` 已完成 Tavily Search/Extract、DeepSeek `deepseek-v4-flash`、百炼 `text-embedding-v4`、真实 Supabase 输入、Inngest durable steps、兼容迁移和专用付费门禁；一次已授权真实冒烟与完整 `test:managed` 均通过。Production 迁移、Vercel Provider 变量、重新部署和真实研究验收仍是合并后的独立敏感发布门禁，当前尚未执行。
