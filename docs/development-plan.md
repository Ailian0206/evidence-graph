...
| 2 | `feat/research-domain` | Research entities, fixtures, project lifecycle, persistence boundary | Local complete; combined post-merge gate pending |
...
Finish the `main` merge into local `feat/research-domain`, run the complete combined gate, then create the module's single Draft PR. Continue `feat/research-workflow` locally while that PR runs CI and Codex review.
