...
- Phase: research workflow Codex review remediation.
- Branch: `feat/research-workflow`.
- Pull request: not opened; the module remains local until review findings and the combined gate pass.
- Active module: deterministic research workflow.
- External provider calls: disabled.
- Production deployment: not configured.
- Local gate: 23 focused workflow tests, lint, and typecheck passed with Node `v22.22.1`; combined post-merge gate pending.
...
| Research workflow | Codex review remediation | Deterministic provider fixtures, resumable workflow, exact citations, retry and cost gates |
...
- Focused `tests/unit/research-workflow.test.ts`: 23 passed after project-isolation fixes.
- Earlier workflow gate passed lint, typecheck, 32 unit tests, and build; its E2E Client Manifest failure is resolved by the merged predecessor's `turbopack.root` fix, so a fresh combined gate is pending.
...
- Review note: research-domain Codex review passed with no blocking findings. Research-workflow review found two Critical and six Important issues; project ownership and cross-project entity reuse are fixed, with remaining findings in progress.
...
Fix the remaining research-workflow review findings through TDD, run the combined `npm run test:ci`, complete final Codex review, and only then open the module's single Draft PR.
