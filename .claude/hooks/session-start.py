...
def _get_task_status(trellis_dir: Path, input_data: dict) -> str:
    ...
    if not active.task_path:
        return (
            ...
            "Next-Action: Classify the current turn. Work inline for small maintenance; "
            "create and plan a Trellis task autonomously for complex work."
        )
    ...
    if task_status == "planning":
        ...
        if missing_complex:
            next_bits.append(
                "Lightweight task can start after PRD self-review; "
                f"complex task must add {', '.join(missing_complex)} before start"
            )
        else:
            next_bits.append("Planning artifacts are present; self-review them before `task.py start`")
        if not jsonl_ready:
            next_bits.append("curate `implement.jsonl` and `check.jsonl` before sub-agent mode start")
        return (
            f"Status: PLANNING\nTask: {task_title}\n"
            f"Present: {present_line}\n"
            f"Next-Action: {'; '.join(next_bits)}. Ask only for the explicit confirmation gates in `AGENT.md`."
        )
