...
```
Phase 1: Plan    → classify, create the task when needed, then write planning artifacts
...
- Simple conversation or small maintenance task: work inline unless persistent task context would materially help.
- Complex task or milestone: create a Trellis task and enter planning without a separate consent prompt.
- Continue from planning into implementation after the artifacts pass self-review. Ask only for the explicit confirmation gates in `AGENT.md`.
...
No active task. Classify the current turn before changing files.
Simple conversation or small maintenance task: work inline unless persistent task context would materially help.
Complex task or milestone: create a Trellis task, plan it, self-review the artifacts, and proceed autonomously. Ask only for the explicit confirmation gates in `AGENT.md`.
[/workflow-state:no_task]
...
Lightweight: `prd.md` can be enough. Complex: finish and self-review `prd.md`, `design.md`, and `implement.md` before `task.py start`.
...
- Implementation waits for `task.py start` after artifact self-review.
...
Goal: classify the request, create a task when needed, and produce the planning artifacts required before implementation.

#### 1.0 Create task `[required · once]`

Create the task directory as soon as a complex task or milestone is identified. The command sets status to `planning`, writes `task.json`, creates a default `prd.md`, and auto-targets the new task when session identity is available:
...
|------|:---:|
| `prd.md` exists | ✅ |
| Planning artifacts pass Agent self-review | ✅ |
...
5. **Execute the commit plan autonomously** for files changed by the current task. Exclude unrecognized dirty files. Ask only if an unrecognized edit overlaps the same lines and prevents a safe commit.
   ```
   ...
   Unrecognized dirty files (NOT included):
     - <file>
     - <file>

   ```

6. Run `git add <files>` + `git commit -m "<msg>"` for each batch in order. Do not amend. Follow the branch and push policy in `AGENT.md` and `AGENTS.md`.

**Rules**:
- No `git commit --amend` anywhere — three-stage three-commit flow (work commits → archive commit → journal commit).
- Never include unrecognized user changes in a commit.
- Do not prompt per commit; report the resulting commits in the final response.

#### 3.5 Wrap-up reminder

After the work commit, run `/trellis:finish-work` (or the equivalent project skill) to archive the task and record the session. Because `session_auto_commit` is disabled in this repository, review the resulting Trellis bookkeeping changes, commit them with a Chinese Conventional Commit, and follow the branch/push policy in `AGENT.md`.
...
- No active task must triage first; complex tasks and milestones create a Trellis task without a separate consent prompt.
