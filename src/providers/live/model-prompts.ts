...
const operationInstructions: Record<ResearchModelOperation, string> = {
  ...
  extract_claims:
    `Extract between 1 and ${MAX_CLAIM_CANDIDATES} decision-relevant atomic claims grounded in the supplied source chunks. When payload.requiredSourceUrls and payload.minimumSourceDomains exist, ground claims in at least that many distinct listed URLs so enough sources can be linked later; never invent a claim merely to satisfy coverage. Preserve qualifications and give every claim a unique candidateId.`,
  link_evidence:
    "Link claims to exact source quotes only; never invent or paraphrase evidence quotes. When payload.minimumSourceDomains exists, return valid evidence from at least that many distinct source URLs. When payload.requiredSourceUrls exists, use only those URLs. Use only supports, rebuts, qualifies, or context as relation values and weak, moderate, or strong as strength values.",
  detect_conflicts:
